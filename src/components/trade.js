...
const Stock = props =>(
<th>
<a href='#' onClick={() => {props.getStock(props.stockname._id) }} >{props.stockname.stock}</a>
</th>
...
export class trade extends Component {
    ...
    stockList() {
        return this.state.stock_array.map(currentstock => {
          return <Stock stockname={currentstock} key={currentstock._id}/>
